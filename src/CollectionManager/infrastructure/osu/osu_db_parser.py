...
def _legacy_parse_osu_db_bytes(data: bytes):
    return get_compiled_osu_db().parse_stream(BytesIO(data))


def _parse_osu_db_bytes(data: bytes):
    if parse_osu_db_bytes_rust is not None:
        return to_namespace(parse_osu_db_bytes_rust(data))
    return _legacy_parse_osu_db_bytes(data)


def _legacy_error_position(data: bytes) -> int:
    stream = BytesIO(data)
    try:
        get_compiled_osu_db().parse_stream(stream)
    except Exception:
        return stream.tell()
    return len(data)
